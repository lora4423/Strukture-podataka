...
dijela zadatka.*/position Delete(position root); int replace(position root); int random(); int levelOrderFile(position root, FILE* filePointer); positionList addToTheEndOfList(positionList head, position newVal); positionList deleteList(positionList head); int replace(position root)
{
	if (root == NULL)
	{
		return 0;
